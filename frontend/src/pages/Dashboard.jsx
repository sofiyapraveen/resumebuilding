...
const Dashboard = () => {
  ...
  const fetchAllResumes=async()=>{
    try{
        ...
        //add completeion percentage to resume
        ...
        <div className={styles.container}>
            <div className={styles.headerWrapper}>
                <div>
                    ...
                    <p className={styles.headerSubtitle}>{allResumes.length > 0 ? `You have ${allResumes.length} resumes${allResumes.length !==1 ? 's' : ''}`
                    : 'start building your professional resume'}</p>
                </div>
  ...
  <div className={styles.emptyStateWrapper}>
    ...
    <button className={styles.createButton} onClick={()=> setOpenCreateModal(true)}>
        ...
        <span className={styles.createButtonContent}>
          ...
          <LucideFilePlus className='group-hover:translate-x-l transition-transform' size={20} />
...
{!loading && allResumes.length > 0 && (
    <div className={styles.grid}>
        ...
        {allResumes.map((resume)=>(
            ...
            // onSelect={() => (`/resume/edit/${resume._id}`)}
